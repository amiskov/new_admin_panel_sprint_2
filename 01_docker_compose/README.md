...
## Установка и запуск
В `Makefile` из текущей директории есть полезные команды:

```sh
# Собрать и запустить контейнеры.
make docker-up

# Запустить миграции, заполнить базу тестовыми данными из `app/db.json.zip`. Будет также создан суперюзер admin/admin.
make docker-fill
